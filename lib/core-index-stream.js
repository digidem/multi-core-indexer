...
const { pDefer, formatKey, defaultByteLength } = require('./utils')
...
class CoreIndexStream extends Readable {
  ...
  async [kPushEntry](index) {
    ...
    if (isIndexed) {
      log(`skipped ${index} ${formatKey(this.#core.key)} (already indexed)`)
      return false
    }
    const block = await this.#core.get(index, { wait: false })
    if (block === null) {
      log(`skipped ${index} ${formatKey(this.#core.key)} (not downloaded)`)
      return false
    }
    ...
    log(
      `push ${index} ${formatKey(this.#core.key)} (buffer free: ${
        this.#readBufferAvailable
      })`
    )
    return true
  }
  ...
  async [kHandleAppend]() {
    log('append', formatKey(this.#core.key))
  ...
  async [kHandleDownload](index) {
    log('download', index, formatKey(this.#core.key))
    clearTimeout(this.#timeoutId)
