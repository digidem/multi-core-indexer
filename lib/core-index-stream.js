...
// How many blocks to read from the core concurrently. Concurrent reads are
// coalesced by hypercore's storage layer into fewer storage round-trips.
//
// 32 is a compromise between two opposing pressures: larger blocks want larger
// batches (more bytes recovered per round-trip), while more cores want smaller
// ones, because each core's stream reads independently and total in-flight
// reads are `cores * READ_BATCH_SIZE`. Indexing throughput in blocks/s,
// 100,000 blocks, Apple M2 Pro (higher is better):
//
...
class CoreIndexStream extends Readable {
  ...
  async #read() {
    ...
    while (this.#readBufferAvailable && this.#index < this.#core.length) {
      // Collect a batch of unprocessed indexes to read concurrently. Reading
      // blocks one-at-a-time is the main indexing bottleneck: each read is a
      // round-trip to the core's storage, whereas concurrent reads are
      // coalesced by hypercore's storage layer (RocksDB) into a single
      // round-trip.
      /** @type {number[]} */
      const indexes = []
      while (
